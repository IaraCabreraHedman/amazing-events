...
.then(dataApi =>{
    console.log(dataApi)
    upcoming = upcomingEvents(dataApi.events, dataApi.currentDate)
    console.log(upcoming)
    createCard(upcoming, cardsContainer)
    showCategories(upcoming, checkboxes)
...
let filtered = upcoming.filter(event => selectedCheck.includes(event.category))
console.log(filtered)
if(selectedCheck.length > 0){
  return createCard(filtered, cardsContainer)
}
else{
return  createCard(upcoming, cardsContainer)
}
...
let filteredString = upcoming.filter(event => event.category.toLocaleLowerCase().startsWith(keyword) );
